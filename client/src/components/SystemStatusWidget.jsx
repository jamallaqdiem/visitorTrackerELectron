      ...
      <div className="fixed bottom-4 left-4 w-72 bg-white rounded-lg shadow-2xl border border-gray-200 z-40 transition-all">
        ...
        >
          <h3 className="text-sm font-bold flex items-center">
            <span className={`w-2.5 h-2.5 rounded-full mr-2 ${status.db_ready ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}></span>
            <Tooltip text="Green means the system is healthy. Red means there is a connection issue with the database."></Tooltip>
            System Health
          </h3>
          <span className="text-[10px] font-mono opacity-70">{isExpanded ? 'CLOSE ▲' : 'VIEW ▼'}</span>
        </div>
        ...
        {isExpanded && (
          <div className="p-4 border-t border-gray-100 bg-white rounded-b-lg">
            ...
            <StatusLine 
              label="Auto-Backup" 
              value={status.last_backup ? new Date(status.last_backup).toLocaleTimeString() : "Pending"} 
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl max-w-xl w-full p-6 border border-gray-300">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-xl font-bold text-gray-800">Support Diagnostics</h2>
